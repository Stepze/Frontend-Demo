...
class Ui_FrontendTest(QtWidgets.QMainWindow):
	def __init__(self, parent):
		...
		self.checkBox.stateChanged.connect(self.checkBoxUpdate)
		...
		self.spinBox.valueChanged.connect(self.checkBoxUpdate)
		...
		self.cpValue = self.spinBox_2.value
		self.cpCounter = 0
	...
	def checkBoxUpdate(self):
		if self.checkBox.isChecked():
			...
			self.spinBox_2.setValue(int(self.spinBox.value()/4))
			
		else:
			if self.cpCounter == 1:
				self.cpCounter = 0
				self.spinBox_2.setValue(self.CPValue)
